...
The POSIX Shell Server Hardening Toolkit is designed to improve server
security, so we take security vulnerabilities in this project very seriously.
...
1. Go to:
   [https://github.com/Bissbert/POSIX-hardening/security/advisories/new](https://github.com/Bissbert/POSIX-hardening/security/advisories/new)
...
4. **Fix Development:** Depending on severity (Critical: <7 days, High: <14
   days, Medium: <30 days)
5. **Security Advisory:** Published when fix is ready
6. **Public Disclosure:** Coordinated with reporter (typically 90 days after
   fix is available)
...
2. **Emergency SSH Access:**
    ...
    - Mitigation: Remove after successful deployment
      (`REMOVE_EMERGENCY_SSH=1`)
...
4. **Backup Contains Sensitive Data:**
    ...
    - Mitigation: Secure `/var/backups/hardening/` with appropriate
      permissions
...
We do not use external package managers (npm, pip, etc.) in the core scripts
...
3. **Vendor Notification:** Affected parties are notified before public
   release
...
We appreciate security researchers who help us improve this project.
