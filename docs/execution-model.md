...
container. Where a fixed defect changed the picture, it links the GitHub
issue that tracked it.
...
flowchart TD
    ...
    CFG -.->|"read by load_config;<br/>flags and environment win"| ORCH
...
`config/defaults.conf` is optional: the orchestrator and the scripts run with
or without it, and a setting given on the command line or in the environment
wins over the file ([#13](https://github.com/Bissbert/POSIX-hardening/issues/13),
[#15](https://github.com/Bissbert/POSIX-hardening/issues/15)). The rollback branch replays the undo actions every
script registers before each change
([#19](https://github.com/Bissbert/POSIX-hardening/issues/19)); see [rollback.md](rollback.md).
...
| `orchestrator.sh` | Runs many scripts in dependency order | `--status`, `--dry-run`, `--all`, `--priority N` and `--script X` run with or without `config/defaults.conf` and honour dependencies |
...
sequenceDiagram
    ...
    S->>S: "load_config config/defaults.conf<br/>(lib/config.sh), FIRST"
    ...
    S->>R: "track_file /etc/thing"
    R->>B: "copy the current file"
    R->>R: "register FILE_RESTORE"
    ...
    alt "exit status 0"
        ...
        Note over R,SYS: "replays every undo action<br/>registered with the transaction"
    end
```

The ordering in the first two lines matters. Configuration must be loaded
before `lib/common.sh`, because `common.sh` makes those same variables
read-only. Every script in `scripts/` and `orchestrator.sh` does this through
`load_config`, which sources the file but keeps any variable that already has
a non-empty value, so the order is command-line flags, then the environment,
then the file, then the built-in defaults.
...
`DRY_RUN=1 sh scripts/05-file-permissions.sh` is a dry run whether or not
`config/defaults.conf` exists, even though the file assigns `DRY_RUN=0`,
because the environment wins over the file
...
All of these are configurable in `config/defaults.conf` or the environment.
`/var/backups/hardening/transactions/` holds the copies `track_file` makes, and
`/var/lib/hardening/rollback_tracked` lists what the open transaction has
already recorded.
...
| `03-kernel-params.sh` | 1 | stopped on `net.ipv4.tcp_congestion_control`, which it names; the rollback removed its block from `sysctl.conf` again |
